...
contract FIONFT is ERC721, Pausable, AccessControl {
    ...
    constructor(
        address[] memory newcustodians
    ) ERC721("FIO Protocol NFT", "FIO") {
        _grantRole(OWNER_ROLE, msg.sender);
        require(newcustodians.length == 4, "Cannot deploy");

        for (uint8 i = 0; i < 4; i++) {
            require(
                !hasRole(CUSTODIAN_ROLE, newcustodians[i]),
                "Custodian already registered"
            );
            require(
                !hasRole(OWNER_ROLE, newcustodians[i]),
                "Owner role cannot be custodian"
            );
    ...
    function tokenURI(
        ...
        return
            string(abi.encodePacked(_baseURI(), attribute[_tokenId], ".json"));
    }
    ...
    function getConsensus(bytes32 hash, uint8 Type) internal returns (bool) {
        ...
        if (approvals[hash].approvals < APPROVALS_NEEDED) {
            require(
                !approvals[hash].approved[msg.sender],
    ...
    function wrapnft(
        ...
        require(
            bytes(domain).length >= 1 && bytes(domain).length < 63,
            "Invalid domain"
        );
        require(bytes(obtid).length > 0, "Invalid obtid");
        require(oracle_count >= 3, "Oracles must be 3 or greater");
        uint256 tokenId = 0;
        bytes32 indexhash = keccak256(
            bytes(abi.encode(ApprovalType.WrapNFT, account, domain, obtid))
        );
    ...
    function unwrapnft(
        string memory fioaddress,
        uint256 tokenId
    ) external whenNotPaused {
        require(
            bytes(fioaddress).length >= 3 && bytes(fioaddress).length <= 64,
            "Invalid FIO Handle"
        );
    ...
    function burnnft(
        ...
        require(oracle_count >= 3, "Oracles must be 3 or greater");
        bytes32 indexhash = keccak256(
            bytes(abi.encode(ApprovalType.BurnNFT, tokenId, obtid))
        );
        if (getConsensus(indexhash, 0)) {
            _burn(tokenId);
            attribute[tokenId] = "";
    ...
    function getApproval(
        bytes memory indexhash
    ) external view returns (uint32, bool, address[] memory) {
        require(indexhash.length > 0, "Invalid obtid");
        address[] memory approvedOracles = new address[](
            approvals[bytes32(indexhash)].approvals
        );
        uint32 c = 0;
        ...
        return (
            approvals[bytes32(indexhash)].approvals,
            approvals[bytes32(indexhash)].complete,
            approvedOracles
        );
    }

    function listDomainsOfOwner(
        address _owner
    ) public view returns (string[] memory ownerTokens) {
    ...
    function regoracle(address account) external onlyRole(CUSTODIAN_ROLE) {
        ...
        bytes32 indexhash = keccak256(
            bytes(abi.encode(ApprovalType.AddOracle, account))
        );
    ...
    function unregoracle(address account) external onlyRole(CUSTODIAN_ROLE) {
        ...
        bytes32 indexhash = keccak256(
            bytes(abi.encode(ApprovalType.RemoveOracle, account))
        );
    ...
    function regcust(address account) external onlyRole(CUSTODIAN_ROLE) {
        ...
        require(account != msg.sender, "Cannot register self");
        bytes32 indexhash = keccak256(
            bytes(abi.encode(ApprovalType.AddCustodian, account))
        );
    ...
    function unregcust(address account) external onlyRole(CUSTODIAN_ROLE) {
        ...
        require(custodian_count > 3, "Must contain 3 custodians");
        bytes32 indexhash = keccak256(
            bytes(abi.encode(ApprovalType.RemoveCustodian, account))
        );
        if (getConsensus(indexhash, 1)) {
